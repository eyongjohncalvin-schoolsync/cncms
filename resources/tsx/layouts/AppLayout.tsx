        ...
        <div className="flex min-h-screen bg-slate-50">
            <aside className="sticky top-0 hidden h-screen w-56 shrink-0 flex-col overflow-y-auto border-r border-slate-200 bg-white md:flex">
                <div className="flex h-14 shrink-0 items-center gap-2 border-b border-slate-200 px-4">
                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-blue-600 text-white shadow-sm shadow-blue-600/20">
                        <IconBroadcast size={18} stroke={1.75} />
                    </span>
                    <span className="text-lg font-bold tracking-tight text-slate-900">CNCMS</span>
                </div>
                            ...
                            <Link
                                ...
                                className={`flex items-center gap-2.5 rounded-md border-l-[3px] px-2.5 py-2 text-sm font-medium transition-colors ${
                                    active
                                        ...
                                        : `border-transparent text-slate-600 ${accent.hover}`
                                }`}
                            >
                                <Icon size={18} stroke={1.75} className={active ? '' : accent.icon} />
                                {t(item.labelKey)}
                            </Link>
                        );
                    })}
            ...
            <div className="flex flex-1 flex-col">
                <header
                    className={`sticky top-0 z-30 flex h-14 items-center justify-between border-b bg-white/90 px-4 backdrop-blur-sm transition-shadow duration-200 ${
                        scrolled ? 'border-slate-200 shadow-sm shadow-slate-900/5' : 'border-transparent'
                    }`}
                >
                            ...
                            <>
                                {auth.user.is_landlord && (
                                    <Link
                                        ...
                                        className="flex items-center gap-1.5 rounded-md px-2 py-1.5 text-sm font-medium text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700"
