 ...
 * against a real, throwaway tenant schema.
 *
 * Unlike ManuscriptCalculateTest's usual pattern (an open, rolled-back
 * `tenant` connection transaction), tests here commit REAL fixtures — they
 * have to: Stancl's DatabaseManager::connectToTenant() unconditionally
 * purges and recreates the `tenant` PDO connection on every
 * tenancy()->initialize() call — including the ones Stancl's
 * QueueTenancyBootstrapper triggers automatically for EVERY queued job
 * (confirmed: Illuminate\Queue\SyncQueue still fires JobProcessing/
 * JobProcessed even under QUEUE_CONNECTION=sync, so this happens even in
 * tests, once per chunk job plus once for the batch's then()/catch()
 * completion job). That purge would silently roll back / disconnect an open
 * outer transaction's uncommitted fixtures before a chunk job ever got to
 * read them. Committing fixtures for real is what makes this test exercise
 * the real multi-connection queue path instead of accidentally testing only
 * the trivial case where everything happens to share one DB session.
 *
 * 2026-08-27: this used to mean committing those real fixtures straight
 * into the live `swecom` tenant schema and manually deleting them again in
 * a bare `finally` block — which is exactly how 1,509 bogus manuscript rows
 * ended up permanently committed against all 446 real swecom customers
 * (a killed test process skipped that `finally` entirely). Fixed by
 * provisioning a brand-new, disposable tenant schema per test method
 * instead (see Tests\Feature\Concerns\UsesDisposableTenant's class doc for
 * the full reasoning) — every fixture below now lives in a schema unique to
 * this one test run, never in `swecom`, so there is no longer any real data
 * for a killed/crashed run to endanger. tearDown() drops the whole schema
 * unconditionally, so individual tests no longer need their own
 * try/finally cleanup at all.
 *
...
class ManuscriptGenerationBatchServiceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tenant = $this->provisionDisposableTenant('zmgb');
    ...
    protected function tearDown(): void
    {
        ...
        // — total, unconditional cleanup regardless of what a test created
        // or how far it got. Never touches `swecom` or any other real
        // tenant. See Tests\Feature\Concerns\UsesDisposableTenant's class
        // doc.
        $this->tenant->delete();
