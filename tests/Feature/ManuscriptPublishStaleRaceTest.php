 ...
 *
 * Like ManuscriptGenerationBatchServiceTest, this test needs real, committed
 * fixtures rather than an open/rolled-back `tenant` transaction (see that
 * class's doc comment for why — Stancl purges the `tenant` PDO connection
 * on every tenancy()->initialize() call, including the ones queued jobs
 * trigger automatically). It uses the same disposable-tenant strategy for
 * the same reason — see Tests\Feature\Concerns\UsesDisposableTenant's class
 * doc: this test used to commit real fixtures into the live `swecom` tenant
 * schema and manually delete them in a `finally` block, which is exactly
 * the pattern that let 1,509 bogus manuscript rows survive a killed test
 * process against all 446 real swecom customers on 2026-08-27. Every
 * fixture below now lives in a schema unique to this one test run instead.
 */
class ManuscriptPublishStaleRaceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tenant = $this->provisionDisposableTenant('zpsr');
